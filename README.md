...
**RAG-as-MCP-Server for AI agents.** Spektr ingests your documents and live conversation streams into a dual knowledge store (vector + graph), then exposes session-aware search tools to any LLM agent over the [Model Context Protocol](https://modelcontextprotocol.io/).
...
```
   Documents (PDF, images)            Live transcripts / streaming text
...
3. **Serves** the index to any LLM agent through an MCP server with six search/listing tools — including session-scoped search that lets agents combine a live conversation with the bulk knowledge base.
...
- **Path B — Live streaming text.** FastAPI endpoint for real-time transcripts (e.g. meetings, agent traces). Each session gets isolated session_id-scoped points.
...
**Two ingestion paths, one query surface.** Bulk and live data live side-by-side in Qdrant/Neo4j; live data is tagged with `session_id` and `is_live` so agents can scope queries per conversation.
