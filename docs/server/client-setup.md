...
- On startup Claude Code prompts once: *"Approve MCP server 'spektr'?"*. Accept → the tools are injected as `mcp__spektr__vector_search`, `mcp__spektr__graph_search`, `mcp__spektr__multi_search`, `mcp__spektr__hybrid_search`, `mcp__spektr__list_documents`, `mcp__spektr__list_document_chunks` (and `mcp__spektr__visual_search` when `MULTIVEC_ENABLED=true`).
