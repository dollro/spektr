...
|`/ingest/transcript`|POST|Bearer `session_token`|Ingest a single transcript chunk; embed → Qdrant immediately, Graphiti as background task|
...
`POST /ingest/transcript` request — `TranscriptChunk`:
...
Live transcript points are written into the same `documents_dense` collection used by Path A. The payload schema is:
...
|`session_id`|`str`|Mirrors `TranscriptChunk.session_id`|
|`speaker`|`str \| None`|From the request|
|`timestamp`|`str`|ISO 8601 from `TranscriptChunk.timestamp`|
|`text_content`|`str`|Raw transcript text|
|`page_number`|`int`|Always `0` for transcript points|
