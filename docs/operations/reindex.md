...
`documents_dense` is recreated with the new vector config on the next ingestion run, by `ingestion/qdrant_setup.py::create_dense_collection` (idempotent — it's a no-op if the collection already exists, which is why the drop above is required first).

**3. Clear CocoIndex tracking.**

Dropping Qdrant alone isn't enough — CocoIndex's PostgreSQL tracking table still thinks every source file was already processed, so a plain `task ingest` would skip everything.

```bash
uv run cocoindex drop --force
```

If that command fails (schema drift, CocoIndex version mismatch), clear the tracking table manually — connect to the `postgres` service and truncate `ragingestion__cocoindex_tracking`, then re-run.
